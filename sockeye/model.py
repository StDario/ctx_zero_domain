...
class ModelConfig(Config):
    ...
    def __init__(self,
                 ...
                 lhuc: bool = False,
                 num_pointers: int = 0,
                 tie_ctx_emb_weights: bool = False) -> None:
        super().__init__()
        ...
        self.tie_ctx_emb_weights = tie_ctx_emb_weights
...
class SockeyeModel:
    ...
    def __init__(self, config: ModelConfig, prefix: str = '') -> None:
        ...
        embed_weight_source, embed_weight_ctx_source, embed_weight_target, out_weight_target = self._get_embed_weights(self.prefix)
        if isinstance(self.config.config_embed_source, encoder.PassThroughEmbeddingConfig):
            ...
            self.ctx_embedding_source = encoder.Embedding(self.config.config_embed_source,
                                                          prefix=self.prefix + "ctx_" + C.SOURCE_EMBEDDING_PREFIX,
                                                          embed_weight=embed_weight_ctx_source, is_source=False)
    ...
    def _get_embed_weights(self, prefix: str) -> Tuple[mx.sym.Symbol, mx.sym.Symbol, mx.sym.Symbol]:
        ...
        if self.config.tie_ctx_emb_weights:
        ...
        return w_embed_source, w_ctx_embed_source, w_embed_target, w_out_target
