...
def add_bucketing_args(params):
    ...
    params.add_argument('--max-ctx-seq-len',
                        type=int,
                        default=100,
                        help='Maximum context sequence length in tokens.')
...
def add_model_parameters(params):
    ...
    model_params.add_argument('--tie_ctx_emb',
                              action='store_true', help='Tie the context embeddings to the source embeddings')

    model_params.add_argument('--model-type', type=str, default=C.MODEL_TYPE_CTX_DEC, choices=C.MODEL_TYPES, 
                                help='Type of context-aware model. {max,avg}_emb_add for DomEmb, ctx_dec for CtxPool (other parameters need to be set)')

    model_params.add_argument('--avg-emb-before',
                              action='store_true', help='average context embeddings before fully connected layer. '
                                                        'Only used when --model-type="avg_emb_{tok,add}"')

    model_params.add_argument('--is-ctx-trg', action="store_true", help='Whether the context consists of target side sentences')

    model_params.add_argument('--use-doc-pool', action="store_true",
                              help="Summarize context with pooling before attention")
    model_params.add_argument('--doc-pool-window', type=int, default=0, help='Document pooling window size - only used with ctx_dec')
    model_params.add_argument('--doc-pool-stride', type=int, default=0, help='Document pooling window stride - only used with ctx_dec')
    model_params.add_argument('--num-ctx-sentences', type=int, default=0,
                              help='Number of previous sentences for the document level model. '
                                   'Used for redefining the max-ctx-input-len parameter based on the pooling parameters')

    model_params.add_argument('--use-avg-pool', action="store_true", help="Use average pooling instead of max - only used for DomEmb models")
    model_params.add_argument('--use-conv', action="store_true", help="Use convolution instead of pooling only")

    model_params.add_argument('--avg-act-type', choices=C.AVG_ACTIVATION_TYPES,
                              default=C.RELU, help='Activation function for after embedding averaging.')

    model_params.add_argument('--avg-dropout', type=float, default=0.1, help='Dropout after averaging embeddings.')
...
def add_inference_args(params):
    ...
    decode_params.add_argument('--max-ctx-input-len',
                               type=int,
                               default=None,
                               help='Maximum context input sequence length. Default: value from model(s).')
    ...
    decode_params.add_argument('--ctx-step-size',
                               default=None,
                               type=int)
