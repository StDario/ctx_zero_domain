...
class ScoringModel(model.SockeyeModel):
    ...
    def _initialize(self,
        ...
        def sym_gen(seq_lens):
            ...
            target_decoded, pointer_scores = self.decoder.decode_sequence(source_encoded, source_encoded_length,
                                                                          source_encoded_seq_len,
                                                                          ctx_source_encoded, ctx_source_encoded_length,
    ...
    def score(self,
        ...
        for batch_no, batch in enumerate(score_iter, 1):
            ...
            for i, (source, ctx_source, target) in enumerate(zip(batch.data[0], batch.data[1], batch.data[2])):
                ...
                source_ids = [int(x) for x in source.asnumpy().flatten().tolist()]
                ...
                if len(scores) > 1:
                    t_output = TranslatorOutput(sentence_no, target_string, None, None, score=score)
                else:
                    t_output = TranslatorOutput(sentence_no, target_string, None, None, score=score[0])
