...
from . import vocab
from .utils import check_condition, smart_open, get_tokens, OnlineMeanAndVariance, compute_max_length_pool
...
def define_ctx_buckets(max_seq_len: int, step=10, ctx_max_seq_len=None, ctx_step=None) -> List[Tuple]:
    ...
    # take the values specific to the main sentence for the context 
...
def define_parallel_buckets(max_seq_len_source: int,
    ...
    ctx_step_size = source_step_size
...
def define_bucket_batch_sizes(buckets: List[Tuple[int, int, int]],
    ...
    for buck_idx, bucket in enumerate(buckets):
        ...
        if batch_by_words:
            ...
            # is the model of type DomEmb - averaging context embeddings
            if is_model_average:
                ...
                batch_size_seq = batch_num_devices * max(1, round((batch_size / (average_seq_len * 3.5 )) / batch_num_devices)) #compute_max_length_pool(average_ctx_seq_len, 10, 10))) / batch_num_devices))
                batch_size_word = batch_size_seq * (average_seq_len ) * 3.5
        else:
    ...
    if batch_by_words:
        ...
        while (bucket_batch_sizes[-1].batch_size * padded_src_seq_len < largest_total_num_source_words or
               bucket_batch_sizes[-1].batch_size * padded_trg_seq_len < largest_total_num_target_words or
                   (not is_model_average and bucket_batch_sizes[-1].batch_size * ctx_padded_seq_len < largest_total_num_context_words)):

            if is_model_average:
                bucket_batch_sizes[-1] = BucketBatchSize(
                    bucket_batch_sizes[-1].bucket,
                    bucket_batch_sizes[-1].batch_size + batch_num_devices,
                    bucket_batch_sizes[-1].average_target_words_per_batch + batch_num_devices * average_seq_len) # + average_ctx_seq_len))
            else:
                bucket_batch_sizes[-1] = BucketBatchSize(
                    bucket_batch_sizes[-1].bucket,
                    bucket_batch_sizes[-1].batch_size + batch_num_devices,
...
def calculate_length_statistics(source_iterables: Sequence[Iterable[Any]],
                                ...
                                max_seq_len_target: int,
                                max_seq_len_ctx_source: int) -> Tuple['LengthStatistics', 'LengthStatistics']:
    """
    ...
    return LengthStatistics(num_sents, mean, std), LengthStatistics(num_sents, mean_ctx, std_ctx)
...
def analyze_sequence_lengths(sources: List[str],
    ...
    length_statistics, length_statistics_ctx = calculate_length_statistics(train_sources_sentences, train_target_sentences,
                                                    max_seq_len_source,
                                                    max_seq_len_target,
    ...
    return length_statistics, length_statistics_ctx
...
def shard_data(source_fnames: List[str],
    ...
    # ctx_source_shard_fnames = [os.path.join(output_prefix, C.SHARD_CTX_SOURCE % i) for i in range(num_shards)]
    ...
    with ExitStack() as exit_stack:
        ...
        # ctx_source_shards = [exit_stack.enter_context(smart_open(f, mode="wt")) for f in ctx_source_shard_fnames]
        ...
        for (sources, target), random_shard_index in zip(parallel_iter(source_readers, target_reader),
            ...
            source_len = len(sources[0][0])
            ctx_source_len = len(sources[0][1])
            target_len = len(target[0])

            buck_idx, buck = get_parallel_bucket(buckets, source_len, target_len, ctx_source_len)
            ...
            for i, line in enumerate(sources):
                sources_shards[i][random_shard_index].write(ids2ctx_strids(line) + "\n")
            target_shards[random_shard_index].write(ids2strids(target[0]) + "\n")
...
class RawParallelDatasetLoader:
    ...
    def load(self,
        ...
        data_source = [np.full((num_samples, source_len, num_factors), self.pad_id, dtype=self.dtype)
                       for (source_len, target_len, _), num_samples in zip(self.buckets, num_samples_per_bucket)]
        data_ctx_source = [np.full((num_samples, ctx_source_len), self.pad_id, dtype=self.dtype)
                           for (source_len, target_len, ctx_source_len), num_samples in
        ...
        for sentno, (sources, target) in enumerate(parallel_iter(source_iterables, target_iterable, skip_blanks=self.skip_blanks), 1):
            ...
            for i, s in enumerate(sources):
                data_source[buck_index][sample_index, 0:source_len, i] = s[0]

            data_ctx_source[buck_index][sample_index, :ctx_source_len] = sources[0][1]
...
def prepare_data(source_fnames: List[str],
    ...
    length_statistics, length_statistics_ctx = analyze_sequence_lengths(source_fnames, target_fname, source_vocabs, target_vocab,
    ...
    buckets = define_parallel_buckets(max_seq_len_source, max_seq_len_target, max_seq_len_ctx_source, bucket_width,
                                      length_statistics.length_ratio_mean, length_statistics_ctx.length_ratio_mean) if bucketing else [
...
def get_data_statistics(source_readers: Optional[Sequence[Iterable]],
    ...
    if source_readers is not None:
        for sources, target in parallel_iter(source_readers, target_reader):
            buck_idx, buck = get_parallel_bucket(buckets, len(sources[0][0]), len(target[0]), len(sources[0][1]))
            data_stats_accumulator.sequence_pair(sources[0], target, buck_idx)
...
def get_validation_data_iter(data_loader: RawParallelDatasetLoader,
    ...
    logger.info("=================================")
    validation_length_statistics, validation_length_statistics_ctx = analyze_sequence_lengths(validation_sources, validation_target,
                                                            source_vocabs, target_vocab,
                                                            max_seq_len_source, max_seq_len_target, max_seq_len_ctx_source)
...
def get_prepared_data_iters(prepared_data_dir: str,
                            ...
                            batch_num_devices: int,
                            permute: bool = True,
                            is_model_average: bool = False) -> Tuple['BaseParallelSampleIter',
    ...
    max_seq_len_ctx_source = config_data.max_seq_len_ctx_source
    ...
    bucket_batch_sizes = define_bucket_batch_sizes(buckets,
                                                   ...
                                                   config_data.data_statistics.average_len_context_per_bucket,
                                                   is_model_average=is_model_average)
...
def get_training_data_iters(sources: List[str],
    ...
    logger.info("===============================")
    # Pass 1: get target/source length ratios.
    length_statistics, length_statistics_ctx = analyze_sequence_lengths(sources, target, source_vocabs, target_vocab,
                                                 max_seq_len_source, max_seq_len_target, max_seq_len_ctx_source)
    ...
    # define buckets
    buckets = define_parallel_buckets(max_seq_len_source, max_seq_len_target, max_seq_len_ctx_source, bucket_width,
                                      length_statistics.length_ratio_mean, length_statistics_ctx.length_ratio_mean, num_ctx_sentences=num_ctx_sentences) if bucketing else [
        (max_seq_len_source, max_seq_len_target)]
...
def get_scoring_data_iters(sources: List[str],
                           ...
                           max_seq_len_source: int,
                           max_seq_len_target: int,
                           max_seq_len_ctx_source: int) -> 'BaseParallelSampleIter':
    """
    ...
    bucket = (max_seq_len_source, max_seq_len_target, max_seq_len_ctx_source)
...
def read_ctx_content(path: str, limit: Optional[int] = None) -> Iterator[List[str]]:
    ...
    with smart_open(path) as indata:
        for i, line in enumerate(indata):
            ...
            line_ctx = ""
            if len(contents) == 1:
                # training sample has no context
                line = contents[0].strip()
            else:
                line_ctx = " <SEP> ".join([c.strip() for c in contents[:-1]])
...
def ids2ctx_strids(ids: Iterable[int]) -> str:
    """
    Returns a string representation of a sequence of integers.
    ...
    :return: String sequence
    """
    # main sentence is at the beginning, should be put at the end
    return C.TOKEN_SEPARATOR.join(map(str, ids[1])) + " <SEP> " + C.TOKEN_SEPARATOR.join(map(str, ids[0]))
...
class SequenceReader(Iterable):
    ...
    def __init__(self,
        ...
        self.bos_id = None
        self.eos_id = None
        self.sentence_sep_or_ctx_id = None
        if vocabulary is not None:
            ...
            assert C.SENTENCE_SEPARATOR_SYMBOL in vocabulary
            self.bos_id = vocabulary[C.BOS_SYMBOL]
            self.eos_id = vocabulary[C.EOS_SYMBOL]
            self.sentence_sep_or_ctx_id = vocabulary[C.SENTENCE_SEPARATOR_SYMBOL]
        else:
    ...
    def __iter__(self):
        for tokens in read_ctx_content(self.path, self.limit):
            if self.vocab is not None:
                sequence = [tokens2ids(tokens[0], self.vocab), tokens2ids(tokens[1], self.vocab)]
            else:
                sequence = [strids2ids(tokens[0]), strids2ids(tokens[1])]
            if len(sequence[0]) == 0:
                yield None
                continue
            if self.add_bos:
                sequence[0].insert(0, self.bos_id)
            if self.add_eos:
                sequence[0].append(self.eos_id)
                sequence[1].insert(0, self.sentence_sep_or_ctx_id)
            yield sequence
...
def get_parallel_bucket(buckets: List[Tuple[int, int, int]],
                        length_source: int,
                        length_target: int,
                        length_ctx_source: int) -> Tuple[Optional[int], Optional[Tuple[int, int, int]]]:
    """
...
class BatchedRawParallelSampleIter(BaseParallelSampleIter):
    ...
    def iter_next(self) -> bool:
        ...
        data = [dataset.source[0], dataset.ctx_source[0], dataset.target[0]]
