...
class TransformerDecoder(Decoder):
    ...
    def decode_sequence(self,
        ...
        ctx_source_bias = None
        if ctx_source_encoded is not None:
            ctx_source_bias = self.valid_length_mask(ctx_source_encoded, ctx_source_encoded_lengths)
        ...
        (target, _, _), _, _, target_max_length = self.pos_embedding.encode(target_embed, None, target_embed_max_length, None, None, None)
        # target, _, target_max_length = self.pos_embedding.encode(target_embed, None, target_embed_max_length, None, None, None)

        if self.config.dropout_prepost > 0.0:
            target = mx.sym.Dropout(data=target, p=self.config.dropout_prepost)

        for layer in self.layers:
            target = layer(target, target_bias, source_encoded, source_bias, ctx_source_encoded, ctx_source_bias)
    ...
    def decode_step(self,
        ...
        source_encoded, source_encoded_lengths, ctx_source_encoded, ctx_source_encoded_lengths, *cache = states  # type: ignore
        ...
        new_states = [source_encoded, source_encoded_lengths, ctx_source_encoded, ctx_source_encoded_lengths]
        ...
        for layer, layer_cache in zip(self.layers, layer_caches):
            target = layer(target, target_bias, source_encoded, source_bias, ctx_source_encoded, ctx_source_bias, layer_cache)
    ...
    def init_states(self,
        ...
        :return: List of symbolic initial states.
        """
        if utils.is_model_average(self.config.model_type):
