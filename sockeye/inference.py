...
class InferenceModel(model.SockeyeModel):
    ...
    def __init__(self,
        ...
        self.avg_emb_ff = transformer.TransformerFeedForward(
    ...
    def initialize(self, max_batch_size: int, max_input_length: int, max_ctx_input_length: int, get_max_output_length_function: Callable):
        """
    ...
    def _get_encoder_module(self) -> Tuple[mx.mod.BucketingModule, int]:
        ...
        def sym_gen(bucket_key: Tuple[int, int]):
            ...
            ctx_source_pad_indicator = ctx_source != C.PAD_ID
            ctx_source_pad_indicator = mx.sym.expand_dims(ctx_source_pad_indicator, axis=2)
            ...
            ctx_source_embed_max, ctx_source_embed_avg = None, None
            ...
            if self.config.config_encoder.model_type == C.MODEL_TYPE_AVG_EMB_TOK or self.config.config_encoder.model_type == C.MODEL_TYPE_MAX_EMB_TOK:
                ...
                if self.config.config_encoder.avg_emb_before:
                    ctx_source_embed = self.avg_emb_ff(ctx_source_embed)

                if self.config.config_encoder.model_type == C.MODEL_TYPE_MAX_EMB_TOK:
                    ctx_source_embed = mx.sym.max(ctx_source_embed, axis=1, keepdims=True)
                else:
                    ctx_source_embed = mx.sym.broadcast_mul(ctx_source_embed, ctx_source_pad_indicator)
                    ctx_sum = mx.sym.sum(ctx_source_embed, axis=1, keepdims=True)
                    ctx_source_embed = mx.sym.broadcast_div(ctx_sum,
                                                            ctx_source_length.expand_dims(axis=1).expand_dims(axis=2))
                    
                if not self.config.config_encoder.avg_emb_before:
                    ctx_source_embed = self.avg_emb_ff(ctx_source_embed)

                source_embed = mx.sym.Concat(ctx_source_embed, source_embed, dim=1)
                source_embed_seq_len += 1
            elif self.config.config_encoder.model_type == C.MODEL_TYPE_AVG_EMB_ADD or self.config.config_encoder.model_type == C.MODEL_TYPE_MAX_EMB_ADD or self.config.config_encoder.model_type == C.MODEL_TYPE_AVG_MAX_EMB_ADD:

                if self.config.config_encoder.avg_emb_before:
                    ctx_source_embed = self.avg_emb_ff(ctx_source_embed)

                if self.config.config_encoder.model_type == C.MODEL_TYPE_MAX_EMB_ADD:
                    ctx_source_embed = mx.sym.broadcast_mul(ctx_source_embed, ctx_source_pad_indicator)
                    ctx_source_embed = mx.sym.max(ctx_source_embed, axis=1, keepdims=True)
                elif self.config.config_encoder.model_type == C.MODEL_TYPE_AVG_EMB_ADD:
                    ctx_source_embed = mx.sym.broadcast_mul(ctx_source_embed, ctx_source_pad_indicator)
                    ctx_sum = mx.sym.sum(ctx_source_embed, axis=1, keepdims=True)
                    ctx_source_embed = mx.sym.broadcast_div(ctx_sum, ctx_source_length.expand_dims(axis=1).expand_dims(axis=2))
                elif self.config.config_encoder.model_type == C.MODEL_TYPE_AVG_MAX_EMB_ADD:
                    ctx_source_embed_max = mx.sym.max(ctx_source_embed, axis=1, keepdims=True)
                    print('Model type not implemented')
                    raise Exception

                if not self.config.config_encoder.avg_emb_before:
                    if ctx_source_embed_avg is None and ctx_source_embed_max is None:
                        ctx_source_embed = self.avg_emb_ff(ctx_source_embed)
                    else:
                        ctx_source_embed_avg = self.avg_emb_ff(ctx_source_embed_avg)
                        ctx_source_embed_max = self.avg_emb_ff(ctx_source_embed_max)

                if ctx_source_embed_avg is None and ctx_source_embed_max is None:
                    source_embed = mx.sym.broadcast_add(source_embed, ctx_source_embed)
                else:
                    source_embed = mx.sym.broadcast_add(source_embed, ctx_source_embed_max)
                    source_embed = mx.sym.broadcast_add(source_embed, ctx_source_embed_avg)
    ...
    def _get_encoder_data_shapes(self, bucket_key: Tuple[int, int], batch_size: int) -> List[mx.io.DataDesc]:
        """
        ...
        return [mx.io.DataDesc(name=C.SOURCE_NAME,
                               shape=(batch_size, bucket_key[0], self.num_source_factors),
                               layout=C.BATCH_MAJOR),
                mx.io.DataDesc(name='ctx_' + C.SOURCE_NAME,
                               shape=(batch_size, bucket_key[1]),
                               layout=C.BATCH_MAJOR)
                ]

    @lru_cache(maxsize=None)
    def _get_decoder_data_shapes(self, bucket_key: Tuple[int, int, int], batch_beam_size: int) -> List[mx.io.DataDesc]:
        """
    ...
    def run_encoder(self,
                    ...
                    source_max_length: int, ctx_source_max_length: int) -> Tuple['ModelState', mx.nd.NDArray]:
        """
        ...
        batch = mx.io.DataBatch(data=[source, ctx_source],
                                ...
                                provide_data=self._get_encoder_data_shapes((source_max_length, ctx_source_max_length), batch_size))
...
def load_models(context: mx.context.Context,
    ...
    for inference_model in models:
        inference_model.initialize(batch_size, max_input_len, max_ctx_input_len, get_max_output_length)
...
class TranslatorInput:
    ...
    def __str__(self):
        return 'TranslatorInput(%s, %s, %s, factors=%s, constraints=%s, avoid=%s)' \
            % (self.sentence_id, self.tokens, self.ctx_tokens, self.factors, self.constraints, self.avoid_list)
    ...
    def chunks(self, chunk_size: int) -> Generator['TranslatorInput', None, None]:
        ...
        for chunk_id, i in enumerate(range(0, len(self), chunk_size)):
            ...
            yield TranslatorInput(sentence_id=self.sentence_id,
                                  ...
                                  ctx_tokens=[C.SENTENCE_SEPARATOR_SYMBOL] + self.ctx_tokens,
                                  factors=factors,
...
def make_input_from_multiple_strings(sentence_id: SentenceId, strings: List[str]) -> TranslatorInput:
    ...
    ctx_tokens = []
    contents = strings[0].split('<SEP>')
    if len(contents) > 1:
        ctx_tokens = list(data_io.get_tokens("<SEP>".join(contents[:-1])))
    tokens = list(data_io.get_tokens(contents[-1]))
    ...
    return TranslatorInput(sentence_id=sentence_id, tokens=tokens, ctx_tokens=ctx_tokens, factors=factors)
...
class Translator:
    ...
    def __init__(self,
        ...
        if bucket_source_width > 0:
            self.buckets_source = data_io.define_ctx_buckets(self._max_input_length, step=bucket_source_width,
                                                             ctx_max_seq_len=self.max_ctx_input_length, ctx_step=ctx_step_size)
        else:
    ...
    def _get_inference_input(self,
        ...
        if bucket_key is None:
            print(max_tok, max_ctx_tok, self.buckets_source)

        source = mx.nd.zeros((len(trans_inputs), bucket_key[0], self.num_source_factors), ctx=self.context)
        ...
        for j, trans_input in enumerate(trans_inputs):
            ...
            max_output_lengths.append(self.models[0].get_max_output_length(data_io.get_ctx_bucket(num_tokens, num_ctx_tokens, self.buckets_source)[0]))
        ...
        return source, bucket_key[0], ctx_source, bucket_key[1], restrict_lexicon, raw_constraints, raw_avoid_list, \
    ...
    def _encode(self, sources: mx.nd.NDArray, source_length: int, ctx_source: mx.nd.NDArray, ctx_source_length: int) -> Tuple[List[ModelState], mx.nd.NDArray]:
        """
        ...
        for model in self.models:
            state, ratio = model.run_encoder(sources, ctx_source, source_length, ctx_source_length)
    ...
    def _beam_search(self,
        ...
        model_states, estimated_reference_lengths = self._encode(source, source_length, ctx_source, ctx_source_length)
        ...
        if self.use_doc_pool:
            ctx_source_length = utils.compute_max_length_pool(ctx_source_length, self.pool_window,
                                                              self.pool_stride) + 1
