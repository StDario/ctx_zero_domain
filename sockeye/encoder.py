...
class AddSinCosPositionalEmbeddings(PositionalEncoder):
    ...
    def encode(self,
        ...
        if ctx_data is not None:
            ctx_positions = mx.sym.arange(0, ctx_seq_len)
            ctx_embedding = self.encode_positions(ctx_positions, ctx_data)
        else:
            ctx_embedding = ctx_data

        return (embedding, ctx_embedding, ctx_seq_len), data_length, ctx_data_length, seq_len
...
class EncoderSequence(Encoder):
    ...
    def encode_with_ctx(self,
        ...
        """
        for encoder in self.encoders:
            (data, ctx_data, ctx_seq_len), data_length, ctx_data_length, seq_len = encoder.encode(data, data_length,
                                                                                                seq_len, ctx_data,
                                                                                                ctx_data_length,
                                                                                                ctx_seq_len)
...
class TransformerEncoder(Encoder, mx.gluon.HybridBlock):
    ...
    def __init__(self,
        ...
        with self.name_scope():
            ...
            if self.config.model_type == C.MODEL_TYPE_CTX_DEC:
                for i in range(config.num_layers - 1):
                    self.layers.add(transformer.TransformerSharedEncoderBlock(config, prefix="%d_" % i))
                self.layers.add(transformer.TransformerCtxEncoderBlock(config, prefix="%d_" % (config.num_layers - 1)))
            else:
            ...
            self.ctx_final_process = transformer.TransformerProcessBlock(sequence=config.preprocess_sequence,
                                                                     dropout=config.dropout_prepost,
                                                                     prefix="ctx_final_process_")

            self.ctx_conv_weight = mx.sym.Variable('%sctx_conv_weight' % prefix)
            self.ctx_conv_bias = mx.sym.Variable('%sctx_conv_bias' % prefix)
    ...
    def _encode_with_ctx(self,
        ...
        if self.config.use_doc_pool:
            # -1 because of removing BOS symbol, and +1 because of adding it back after pooling
            ctx_seq_len = utils.compute_max_length_pool(ctx_seq_len - 1, self.config.doc_pool_window, self.config.doc_pool_stride) + 1
        ...
        if self.config.use_doc_pool:
            bos_sym = mx.sym.slice(ctx_data, begin=(None, 0, None), end=(None, 1, None))
            ctx_data = mx.sym.slice(ctx_data, begin=(None, 1, None), end=(None, None, None))

            if self.config.use_conv:
                ctx_data = mx.symbol.swapaxes(ctx_data, dim1=1, dim2=2)
                ctx_data = mx.sym.Convolution(data=ctx_data, num_filter=self.config.model_size,
                                              stride=(int(self.config.doc_pool_stride / 2)),
                                              kernel=(int(self.config.doc_pool_window / 2)),
                                              weight=self.ctx_conv_weight,
                                              bias=self.ctx_conv_bias, pad=(int(self.config.doc_pool_window / 2) - 1),
                                              layout='NCW')
                ...
                ctx_data = mx.symbol.Pooling(ctx_data, pool_type=pool_op, kernel=(2),
                                             stride=(2),
                                             pad=(1, 2),
                ...
                ctx_data = mx.symbol.Pooling(ctx_data, pool_type=pool_op, kernel=(self.config.doc_pool_window), stride=(self.config.doc_pool_stride), pad=(self.config.doc_pool_window - 1, 1),
                                             pooling_convention='valid',
                                             layout='NWC')

            ctx_data = mx.sym.concat(bos_sym, ctx_data, dim=1)
            ctx_data_length = (ctx_data_length - self.config.doc_pool_window + 2 * (self.config.doc_pool_window - 1)) / self.config.doc_pool_stride + 1 + 1 # another one for extra symbol
            ctx_data_length = F.cast(ctx_data_length, dtype='float32')


        bias = F.expand_dims(self.valid_length_mask(data, data_length), axis=1)
        ctx_bias = F.expand_dims(self.valid_length_mask(ctx_data, ctx_data_length), axis=1)
        ...
        data = self.final_process(data, None)
        if self.config.model_type != "ctx_enc" and self.config.model_type != C.MODEL_TYPE_AVG_EMB_TOK \
                and self.config.model_type != C.MODEL_TYPE_MAX_EMB_TOK and self.config.model_type != C.MODEL_TYPE_AVG_EMB_ADD \
                and self.config.model_type != C.MODEL_TYPE_MAX_EMB_ADD and self.config.model_type != C.MODEL_TYPE_AVG_DOC_ATTN and self.config.model_type != C.MODEL_TYPE_AVG_MAX_EMB_ADD:
            ctx_data = self.ctx_final_process(ctx_data, None)

        return data, ctx_data, ctx_seq_len
    ...
    def encode(self,
        ...
        :return: Encoded versions of input data data, data_length, seq_len.
        """
        return self._encode_with_ctx(mx.sym, data, data_length, ctx_data, ctx_data_length, ctx_seq_len), data_length, ctx_data_length, seq_len
