...
def is_model_average(model_type: str):

    return model_type == C.MODEL_TYPE_AVG_EMB_ADD or model_type == C.MODEL_TYPE_MAX_EMB_ADD \
           or model_type == C.MODEL_TYPE_AVG_EMB_TOK or model_type == C.MODEL_TYPE_MAX_EMB_TOK \
           or model_type == C.MODEL_TYPE_AVG_MAX_EMB_ADD
...
def compute_max_length_pool(max_len, window, stride):

    max_len = int((max_len - window + 2 * (window - 1)) / stride + 1)
    return max_len
...
def get_ctx_tokens(line: str) -> Tuple[List[str], List[str]]:
    ...
    if len(contents) > 1:
        ctx_tokens = contents[0].split()
    tokens = contents[-1].split()

    return (tokens, ctx_tokens)
