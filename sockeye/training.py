    ...
    def _initialize(self,
        ...
        def sym_gen(seq_lens):
            ...
            if self.config.config_encoder.model_type == C.MODEL_TYPE_AVG_EMB_TOK or self.config.config_encoder.model_type == C.MODEL_TYPE_MAX_EMB_TOK:
                ...
                # apply feed-forward network before averaging embeddings
                ...
                if self.config.config_encoder.model_type == C.MODEL_TYPE_MAX_EMB_ADD:
                    ctx_source_embed = mx.sym.broadcast_mul(ctx_source_embed, ctx_source_pad_indicator)
                    ctx_source_embed = mx.sym.max(ctx_source_embed, axis=1, keepdims=True)
                    ctx_source_embed = mx.sym.Dropout(ctx_source_embed, p=self.config.config_encoder.avg_dropout)
                    ...
                    print('Model type not fully implemented')
                    raise Exception
            ...
            elif self.config.config_encoder.model_type == C.MODEL_TYPE_AVG_DOC_ATTN:

                if self.config.config_encoder.avg_emb_before:
                    ctx_source_embed = self.avg_emb_ff(ctx_source_embed)

                ctx_source_embed = mx.sym.mean(ctx_source_embed, axis=1, keepdims=True)
            ...
            target_decoded, pointer_scores = self.decoder.decode_sequence(source_encoded, source_encoded_length, source_encoded_seq_len,
                                                          ctx_source_encoded, ctx_source_encoded_length, ctx_source_encoded_seq_len,
